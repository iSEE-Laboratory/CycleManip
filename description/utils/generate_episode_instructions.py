...
def filter_instructions(instructions: List[str], episode_params: Dict[str, str]) -> List[str]:
    ...
    matching the available episode parameters. Accept instructions that
    have fewer placeholders than available parameters.
    """
    ...
    for instruction in instructions:
        ...
        # Accept if all placeholders in the instruction are available in episode_params
        # and we don't care about extra parameters in episode_params
        if set(placeholders).issubset(set(stripped_episode_params.keys())):
...
def get_loop_times(task_name: str, setting: str) -> List[int]:
    """读取data目录下对应的loop_times.txt,然后返回一个列表"""
    loop_times_file = os.path.join(parent_directory, f"../../data/{task_name}/{setting}/loop_times.txt")
    try:
        with open(loop_times_file, "r") as f:
            ...
            if not content:
                return []
            # 按空格分割并转换为整数
            loop_times = [int(num) for num in content.split()]
        return loop_times
    except FileNotFoundError:
        # print(f"\033[1m[genInstruction]ERROR: Loop times file '{loop_times_file}' not found.\033[0m")
        return None
    except ValueError as e:
        print(f"\033[1m[genInstruction]ERROR: Invalid format in loop times file: {e}\033[0m")
        print(f"File content: {content if 'content' in locals() else 'Unable to read'}")
        return None


def generate_episode_descriptions_inference(task_name: str, episodes: List[Dict[str, str]], max_descriptions: int = 1000000, language_type="instruction_sim") -> List[Dict]:
    """
    ...
    """
    if language_type == "instruction_full":
        task_data = load_task_instructions(task_name+"_full")
        seen_instructions = task_data.get("seen", [])
        unseen_instructions = task_data.get("unseen", [])
    elif language_type == "instruction_sim":
        task_data = load_task_instructions(task_name)
        seen_instructions = task_data.get("seen", [])
        unseen_instructions = task_data.get("unseen", [])
    elif language_type == "instruction_int":
        seen_instructions = ["[num]"] * max_descriptions
        unseen_instructions = ["[num]"] * max_descriptions
    else:
        raise Exception(f"{language_type} is not valid language type")

    int_instructions = ["[num]"] * max_descriptions
    ...
    for i, episode in enumerate(episodes):
        ...
        filtered_seen_instructions = seen_instructions
        filtered_unseen_instructions = unseen_instructions

        instruction = random.choice(filtered_seen_instructions)
        description = replace_placeholders(instruction, episode)

        seen_episode_descriptions = description

        instruction = random.choice(filtered_unseen_instructions)
        description = replace_placeholders_unseen(instruction, episode)
        unseen_episode_descriptions = description

        int_instructions = random.choice(int_instructions)
        int_instructions = replace_placeholders_unseen(int_instructions, episode)
        
        all_generated_descriptions.append({
            "episode_index": i,
            "seen": seen_episode_descriptions,
            "unseen": unseen_episode_descriptions,
            "instructions_int": int_instructions,
        })
...
def generate_episode_descriptions(task_name: str, episodes: List[Dict[str, str]], max_descriptions: int = 1000000, setting: str = "default") -> List[Dict]:
    """
    Generate descriptions for episodes by replacing placeholders in instructions with parameter values.
    For each episode, filter instructions that have matching placeholders and generate up to
    max_descriptions by replacing placeholders with parameter values.
    ...
    # breakpoint()
    loop_times = get_loop_times(task_name, setting)
    print(f"Loaded loop times: {loop_times}")
    ...
    for i, episode in enumerate(episodes):
        ...
        #     # 随机选取一种statement，然后将循环信息加在每个instruction的末尾
        #     epi_loop_time = loop_times[i] 
        #     # 加在每个filtered_seen_instructions的末尾
        #     filtered_seen_instructions = [inst[:-1] + " " + random.choice(loop_statements).replace("{N}", str(epi_loop_time)) + "." for inst in filtered_seen_instructions]
        #     filtered_unseen_instructions = [inst + " " + random.choice(loop_statements).replace("{N}", str(epi_loop_time)) + "." for inst in filtered_unseen_instructions]
...
if __name__ == "__main__":
    ...
    results = generate_episode_descriptions(args.task_name, episodes, args.max_num, args.setting)
