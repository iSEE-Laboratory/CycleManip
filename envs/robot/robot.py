...
class Robot:
    ...
    def reset(self, scene, need_topp=False, **kwargs):
        ...
        # Ensure communication_flag and planners exist before using them.
        if not hasattr(self, "communication_flag"):
            # default to False (single-process planners) if not set yet
        ...
        if self.communication_flag:
            if hasattr(self, "left_conn") and self.left_conn:
                try:
                    ...
                    # If pipe communication fails, fall back to local planner setup
                    pass
            ...
            need_setup = (
                ...
                or (not isinstance(getattr(self, "left_planner", None), CuroboPlanner))
