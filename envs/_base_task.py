...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        # ------------- key objects begin --------------
        self.key_objects = {}  # Dictionary to store key objects: {"object_name": actor_object}
        # ------------- key objects end --------------
        ...
        self.dont_stop = kwags.get("dont_stop", False)  # Add dont_stop parameter
    ...
    def set_key_objects(self, objects_dict):
        """
        Set key objects for 6D pose tracking.
        Should be called after load_actors() in the subclass.
        
        Args:
            objects_dict: Dictionary mapping object names to actor objects
                         e.g., {"bottle": self.bottle, "cup": self.cup}
        """
        self.key_objects = objects_dict

    def record_loop_metric(self):
        """
        这个函数是专门为了记录循环任务的循环次数设计的，会在每一次get_obs之后调用
        将一些 判定循环 所需要的 信息 存储在 self.loop_metric 中，供后续判定循环使用

        子类需要根据具体任务实现这个函数

        例如
            self.loop_metric['bottle_height'].append(bottle.get_pose().p[2])
        """
        pass

    def analyze_loop_metric(self):
        """
        这个函数是专门为了分析循环任务的循环次数设计的，会在一次测评结束后调用
        根据 self.loop_metric 中存储的信息，分析出循环的次数、开始和结束的时间点等

        子类需要根据具体任务实现这个函数，必须返回一个字典, 必须包含以下字段

        例如
            - 通过分析 self.loop_metric['bottle_height'] 的变化，判断瓶子被摇晃的次数
            - 记录每次摇晃的开始和结束帧数
        """
        loop_info_example = {
            "loop_times": -1,
        ...
        return loop_info_example

    def save_history_obs(self, save_path):
    ...
    def get_obs(self):
        ...
        # ------------- binary --------------
        self.history_obs.append(deepcopy(pkl_dic))
        # ------------- binary --------------

        # ------------- loop --------------
        self.record_loop_metric()
        pkl_dic["loop_counter"] = self.loop_counter
        # ------------- loop --------------

        # ------------- key objects 6D pose begin --------------
        if self.key_objects:
            object_pos = {}
            for obj_name, obj_actor in self.key_objects.items():
                pose = obj_actor.get_pose()
                # Store as 7-tuple: [x, y, z, qx, qy, qz, qw]
                object_pos[obj_name] = np.array([
                    pose.p[0], pose.p[1], pose.p[2],
                    pose.q[0], pose.q[1], pose.q[2], pose.q[3]
                ])
            pkl_dic["object_pos"] = object_pos
        # ------------- key objects 6D pose end --------------

        pkl_dic["test_time_instruction"] = self.instruction
        return pkl_dic
    ...
    def save_loop_times(self, loop_times=-1):
        if loop_times < 0:
            return
        ...
        with h5py.File(target_file_path, "a") as f:
            if "loop_times" in f:
                del f["loop_times"]
            f.create_dataset("loop_times", data=loop_times, dtype=np.int32)
            # print(f"Loop times saved: {loop_times} in {target_file_path}")
    ...
    def set_instruction(self, instruction=None):
        ...
        print(f"Set instruction: {self.instruction}")
    ...
    def wait(self, duration=1.0, save_freq=-1):
        """
        Wait for a specified duration.
        - duration: The time to wait in seconds.
        - save_freq: Frequency for saving frames during wait
        """
        if not self.plan_success:
            return False
        if duration <= 0:
        ...
        steps = int(duration / self.scene.get_timestep())
        save_freq = self.save_freq if save_freq == -1 else save_freq
        
        # print(f"Waiting for {steps} steps ({duration}s)")
        
        # Take initial picture if save_freq is enabled
        ...
        for i in range(steps):
            ...
            # Save picture at specified frequency
            if save_freq is not None and i % save_freq == 0:
                self._update_render()
                self._take_picture()
                
        # Take final picture if save_freq is enabled
        if save_freq is not None:
            self._take_picture()
