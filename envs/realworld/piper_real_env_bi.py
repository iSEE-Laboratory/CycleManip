...
class PiperRealEnv:
    ...
    def take_action(self, action: np.ndarray) -> None:
        ...
        # 发送指令到机器人
        self.robot_left.command_joint_state(action[:7], "state")
        self.robot_right.command_joint_state(action[7:14], "state")
    ...
    def reset(self) -> None:
        ...
        for i in range(100):
            alpha = (i + 1) / 100.0
            interp_pos_left = (1 - alpha) * self.robot_left.get_joint_state() + alpha * self.init_pos[:7]
            self.robot_left.command_joint_state(interp_pos_left, "state")
            interp_pos_right = (1 - alpha) * self.robot_right.get_joint_state() + alpha * self.init_pos[7:]
