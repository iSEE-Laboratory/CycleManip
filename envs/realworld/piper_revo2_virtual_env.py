"""
Piper Revo2 Virtual Environment for Testing Real World Pipeline
双手Piper虚拟环境,用于测试真机部署 pipeline
...
class PiperRevo2VirtualEnv:
    """
    虚拟的双手 Piper 真机环境，用于测试真机部署 pipeline
    ...
    def _load_data_to_memory(self):
        ...
        if self.endpose_array is not None:
            cprint(f"  - endpose_array: {self.endpose_array.shape}", "white")
    ...
    def get_obs(self) -> Dict[str, Any]:
        ...
        Returns:
            observation: 包含 joint_action, pointcloud, endpose 等的字典
        """
        ...
        if self.policy == "pi0":
            ...
            # DP3 需要 joint_action.vector, pointcloud, endpose
            obs = {
                "joint_action": {},
                "pointcloud": None,
                "endpose": None,
            ...
            # 3. 读取末端位姿 (14维)
            if self.endpose_array is not None:
                obs["endpose"] = np.array(self.endpose_array[step]).astype(np.float32)
    ...
    def take_action(self, action: np.ndarray) -> None:
        ...
        Args:
            action: 动作向量 [左臂6+左手6+右臂6+右手6] = 24维
        """
        self.take_action_cnt += 1
        
        cprint(f"🎮 Step {self.take_action_cnt}/{self.step_lim}: 执行动作 {action.shape}", "magenta")
    ...
    def __repr__(self) -> str:
        return (f"PiperRevo2VirtualEnv(data={self.data_path.name}, "
