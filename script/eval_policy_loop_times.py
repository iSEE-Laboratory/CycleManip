...
def main(usr_args):
    ...
    st_seed, suc_num, success_episodes, failure_episodes, loop_info = eval_policy(task_name,
                                   TASK_ENV,
                                   args,
                                   model,
                                   st_seed,
    ...
    # ------------- loop --------------
    if len(loop_info) > 0:
        loop_file_path = os.path.join(save_dir, f"loop_metric.txt")
        with open(loop_file_path, "w") as file:
...
def eval_policy(task_name,
    ...
    use_random_loop = args["loop"]["is_random"]
    if use_random_loop:
        print(f"use random loop times")
        loop_times_min = args["loop"]["random_min"]
        loop_times_max = args["loop"]["random_max"]
        print(f"loop times range: [{loop_times_min}, {loop_times_max}]")
    else:
        print(f"use fixed loop times")
        loop_times = args["loop"]["loop_times"]
    ...
    while succ_seed < test_num:
        ...
        if (not expert_check) or (TASK_ENV.plan_success and TASK_ENV.check_success()):
            succ_seed += 1
            suc_episode_list.append(now_id)
        else:
            now_seed += 1
            args["render_freq"] = render_freq
            continue
        ...
        ################ loop instruction ###############
        if use_random_loop:
            this_loop_time = np.random.randint(loop_times_min, loop_times_max + 1)
        else:
            this_loop_time = loop_times
        ...
        if TASK_ENV.eval_video_path is not None:
            ffmpeg = subprocess.Popen(
                [
                    ...
                    "-crf",
                    "23",
                    f"{TASK_ENV.eval_video_path}/episode{TASK_ENV.test_num}-loop{this_loop_time}.mp4",
                ],
        ...
        if succ:
            ...
            fail_episode_list.append(now_id)
    ...
    return now_seed, TASK_ENV.suc, suc_episode_list, fail_episode_list, loop_info
