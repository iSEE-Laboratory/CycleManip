...
def main(usr_args):
    ...
    save_dir = Path(f"replay_result/{task_name}/{policy_name}/{exp_tag}_{str(checkpoint_num)}/{current_time}")
...
def eval_policy(task_name,
    ...
    if use_random_loop:
        ...
        # 创建一个从min到max的列表: min min+1 min+2 ... max
        loop_times = []
        for i in range(loop_times_min, loop_times_max + 1):
            loop_times.append(i)
    else:
        print(f"use fixed loop times")
        loop_times = [args["loop"]["loop_times"]] # 为了统一，也用列表存储
