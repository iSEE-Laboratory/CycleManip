...
class RobotImageDataset(BaseImageDataset):
    ...
    def __init__(
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            ...
            keys=["head_camera", "state", "action", "instruction_int"],
        )
        ...
        self.buffers = {
            ...
            for k, v in self.sampler.replay_buffer.items() if k not in ["instruction_int"] 
        }
    ...
    def _sample_to_data(self, sample):
        ...
        data = {
            ...
            "instruction_int": sample["instruction_int"]
        }
        return data
    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        if isinstance(idx, slice):
            ...
            if "instruction_int" in self.sampler.replay_buffer or "instruction_sim" in self.sampler.replay_buffer:
                if "instruction_int" in self.sampler.replay_buffer:
                    self.buffers_torch["instruction_int"] = batch_sample_instruction(
                            self.sampler.replay_buffer["instruction_int"],
                            self.sampler.indices,
                            idx,
                    )
                else:
                    self.buffers_torch["instruction_int"] = batch_sample_instruction(
                            self.sampler.replay_buffer["instruction_sim"],
                            self.sampler.indices,
                            idx,
                    )    
    ...
    def postprocess(self, samples, device):
        ...
        return {
            ...
            "action": action,  # B, T, D
            "instruction_int": samples["instruction_int"]
        }
...
def batch_sample_instruction(
    ...
    for i in numba.prange(len(idx)):
        buffer_start_idx, buffer_end_idx, sample_start_idx, sample_end_idx = indices[idx[i]]
        data.append(input_arr[buffer_start_idx][0])
    return data
