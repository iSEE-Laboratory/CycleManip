...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        for key, value in root["data"].items():
            assert value.shape[0] == root["meta"]["episode_ends"][-1], (key, value.shape[0], root["meta"]["episode_ends"][-1])
        self.root = root
    ...
    def copy_from_store(
        ...
        if store is None:
            ...
            data = dict()
            for key in keys:
                if key not in src_root["data"]:
                    print(f"[Warning] Key '{key}' not found in src_root['data'], skipped.")
                    continue
        ...
        for key, value in buffer.items():
            try:
                ...
                cprint(
                    f"Replay Buffer: {key}, length {len(value)}, shape {value.shape}",
                    "green",
                )
        cprint("--------------------------", "green")
        return buffer
