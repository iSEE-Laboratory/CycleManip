    ...
    def _normalize_impl(self, x, forward=True):
        if isinstance(x, dict):
            ...
            for key, value in x.items():
                params = self.params_dict.get(key, self.params_dict.get("_default"))
                if params is None:
                    raise RuntimeError(f"Missing normalization parameters for key '{key}'")

                # ✅ 如果 value 是 list 或 tuple，逐个处理元素
                if isinstance(value, (list, tuple)):
                    result[key] = [
                        _normalize(v, params, forward=forward)
                        if isinstance(v, (torch.Tensor, np.ndarray))
                        else v  # 如果里面是别的类型就保持原样
                        for v in value
                    ]
                else:
