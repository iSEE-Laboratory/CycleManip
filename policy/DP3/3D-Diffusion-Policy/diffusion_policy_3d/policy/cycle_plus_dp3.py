...
from diffusion_policy_3d.model.vision.pointnet_extractor_plus import DP3EncoderPlus, create_mlp
from diffusion_policy_3d.model.language import LanguageEncoderFinetune, LanguageEncoderFrozen, LanguageEncoderMulti

class CyclePlusDP3(BasePolicy):
    """
    二分观测采样的DP3模型
    - 观测：二分采样的长历史序列
    - 动作：未来的连续动作序列
    ...
    def __init__(
        ...
        **kwargs,
        ...
        short_joint_dim = obs_encoder.short_joint_dim
        obs_state_dim = obs_encoder.state_dim
        ...
        if language_encoder_type in language_encoder_classes:
            language_encoder = language_encoder_classes[language_encoder_type](language_embed_dim=512, output_projection_dim=short_joint_dim, language_keys=language_keys)
        ...
        if obs_as_global_cond:
            if "cross_attention" in self.condition_type:
                ...
                if language_encoder_type in language_encoder_classes:
                    global_cond_dim = obs_feature_dim + language_feature_dim
                else:
        ...
        if self.new_multitask:
            ...
            if self.multi_selftask:
                self.selfcounter_decoder = nn.Sequential(*create_mlp(obs_state_dim, 10, [], activation_fn=nn.ReLU))
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        nobs_features, pn_feat, joint_feat, state_feat = self.obs_encoder(this_nobs)
    ...
    def compute_loss(self, batch):
        ...
        if self.obs_as_global_cond:
            ...
            nobs_features, pn_feat, short_state_feat, state_feat = self.obs_encoder(this_nobs)
            if self.language_encoder is not None:
                language_features = self.language_encoder(this_nobs)   
        ...
        if self.new_multitask:
            if self.multi_selftask:
                selfcounter_from_now_pred = self.selfcounter_decoder(state_feat)
                gt = obs_dict["loop_now"]
