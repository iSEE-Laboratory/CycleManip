...
class CycleDP3(BasePolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        obs_encoder = PC_STATE_Encoder(
            observation_space=obs_dict,
            img_crop_shape=crop_shape,
            out_channel=encoder_output_dim,
            encoder_cfg=pointcloud_encoder_cfg,
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        nobs_features = self.obs_encoder(this_nobs, (batch_size, self.obs_horizon))
        ...
        if "cross_attention" in self.condition_type:
            ...
            if self.language_encoder is not None:
                global_cond = torch.cat([global_cond, language_features], dim=-1)
    ...
    def compute_loss(self, batch):
        ...
        if self.obs_as_global_cond:
            if isinstance(nobs["agent_pos"], torch.Tensor):
                nobs_for_reshape = {k: v for k, v in nobs.items() if "instruction" not in k and "agent_pos" not in k}
                agent_pos = nobs["agent_pos"]
                this_nobs = dict_apply(nobs_for_reshape, lambda x: x.reshape(-1, *x.shape[2:]))
                this_nobs["agent_pos"] = agent_pos
            else:
                nobs_for_reshape = {k: v for k, v in nobs.items() if "instruction" not in k}
                this_nobs = dict_apply(nobs_for_reshape, lambda x: x.reshape(-1, *x.shape[2:]))
        ...
        if self.multi_task_counter:
            counter_from_now_pred = self.counter_decoder(nobs_features)
