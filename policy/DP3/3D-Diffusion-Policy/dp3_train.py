...
class TrainDP3Workspace:
    ...
    def get_policy_and_runner(self, cfg, usr_args):
        ...
        sampler_strategy = cfg.task.dataset.sampler_strategy # usr_args['sampler_strategy']

        env_runner = RobotRunner(n_obs_steps=n_obs_steps, n_action_steps=n_action_steps, sampler_strategy=sampler_strategy)

        if not cfg.policy.use_pc_color:
            ckpt_file = pathlib.Path(
                os.path.join(
                    DP3_ROOT,
                    f"experiments/{usr_args['task_name']}/{usr_args['exp_name']}/seed_{usr_args['seed']}",
                    "wo_color_pc",
