...
class Pi0(_model.BaseModel):
    ...
    def embed_suffix(
        ...
        state_token = self.state_proj(obs.state)[:, None, :] # B,K,C
    ...
    def compute_loss(self,
        ...
        u_t = noise - actions # gt

        # one big forward pass of prefix + suffix at once
        # 将图片、语言token化  返回：token，输入padding mask，自回归mask
        prefix_tokens, prefix_mask, prefix_ar_mask = self.embed_prefix(observation)
        # 将状态+时间嵌入+动作 token化  返回：token，输入padding mask，自回归mask
